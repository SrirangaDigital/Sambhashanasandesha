...
echo "Create database if not exists sandesha CHARACTER SET utf8 COLLATE utf8_general_ci;" | /usr/bin/mysql -u$usr -p$pwd
...
echo "create fulltext index text_index on searchtable (text);" | /usr/bin/mysql -u$usr -p$pwd $db
